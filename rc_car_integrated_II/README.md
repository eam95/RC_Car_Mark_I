...
| Accelerometer | LIS3DH | I2C | Measures acceleration for velocity estimation |
...
| Drive | DC Motor | PWM | Built-in to car, and hooked to motor driver |
...
### Commands Transmitted to Transmitter

Sensor data is packed into a 32-byte string and transmitted over NRF24L01:
...
communication circuits, and the MCU is powered by TMH1205S Traco Isolated DC-DC with an additonal DC/DC to ensure it powering the board at 5V

### Steering Control

The servo motor is controlled by TIM13 Channel 1 at 50Hz with a pulse width
between 1ms and 2ms. The center position corresponds to a at 1.5ms pulse which is valued at 75, with
...
32-byte string, and transmitted wirelessly to the transmitter which forwards
them to the Python GUI over UART.

### Accelerometer Calibration

A calibration mode is triggered by the `CAL` command. In this mode the RC Car
transmits raw accelerometer data (without distance) at the normal 20ms rate
...
3. Navigate to the `rc_car_integrated_II/` folder and import the project
4. Build and flash to the STM32H723ZG Nucleo board
